...
#axv[0][0].hist(m_new,bins=binar, density=True,label='Old', histtype="step", linewidth=2.)
...
#axv[0][1].hist(q_new, bins=np.logspace(np.log10(qmin),np.log10(qMax), 16), histtype='step', linewidth=2., density='True', label=r'Old')
...
#if sp.medium_interpolate == True:
#    axv[0][1].hist(stars.binaries.q[(mask_bin) & (stars.binaries.m1>=sp.m_thres_low) & (stars.binaries.m1<sp.m_thres_high)], bins=np.logspace(np.log10(qmin),np.log10(qmax), nbins), histtype='step', linewidth=2., density='True', linestyle="dotted", label=r'$2 < m < 15 \, \mathrm{M_{\odot}}$')
#axv[0][1].hist(stars.binaries.q[(mask_bin) & (stars.binaries.m1>=sp.m_thres_high)], bins=np.logspace(np.log10(qmin),np.log10(qmax), nbins), histtype='step', linewidth=2., density='True', linestyle="dotted", label=r'$m > 15 \, \mathrm{M_{\odot}}$' if sp.medium_interpolate else r'$m > 20 \, \mathrm{M_{\odot}}$')

#axv[0][1].hist(q3_new, bins=np.logspace(np.log10(qmin),np.log10(qMax), 10), histtype='step', edgecolor="Blue", linewidth=2., density='True', label=r'M$_3$/M$_1$')

axv[0][1].legend(loc='upper left', fontsize=16)
#plt.title('Binary mass ratio', fontsize=16)
...
#axv[1][0].hist(e_new[e_new[:]>0],bins=np.logspace(np.log10(emin),np.log10(eMax),10), histtype='step', linewidth=2., density='True',label='Old')
...
#plt.title('Eccentricity', fontsize=16)
...
#axv[1][1].plot(aP_LM,expoP_LM,label=f'LM: R2010')
if sp.medium_interpolate == True:
    axv[1][1].plot(aP_MM,expoP_MM,label=f'MM: K2007', linestyle='--')
#axv[1][1].hist(np.log10(P_new[P_new[:]>0]), bins=np.logspace(np.log10(pmin),np.log10(pMax), 10), histtype='step', linewidth=2., density='True',label='Old')
...
#plt.title('Period', fontsize=16)
axv[1][1].tick_params(length=6, labelsize=16)
#axv[1][1].set_xscale('log')   
axv[1][1].set_xlabel(r'$Log(P) [days] $', fontsize=18)
axv[1][1].set_ylabel(r'$[Log(P)]^\pi$', fontsize=18)
#semi-major axes distribution

#plt.subplot(326)
#axv[1][1].plot(aax,expoax,label=r'$\pi$=-0.55')
#axv[0][2].hist(a_new, bins=np.logspace(np.log10(min(a_new[a_new[:]>0])),np.log10(max(a_new[a_new[:]>0])), 10), density = 'True', histtype='step', linewidth=2., label='Old')
...
#print(min(a_new[a_new[:]>0]/(4.848*10.**(-6))))
axv[0][2].tick_params(length=6, labelsize=16)
axv[0][2].legend(fontsize=16)
#plt.title('Semi-major axis', fontsize=16)
...
#axv[1][2].scatter(np.log10(P_new[P_new[:]>0]),e_new[P_new[:]>0],label='Old', s=1)
axv[1][2].scatter(np.log10(stars.binaries.P[mask_bin]),stars.binaries.ecc[mask_bin],label='New', s=1)

axv[1][2].legend(fontsize=16)
#plt.title('Period', fontsize=16)
#axv[1][2].tick_params(length=6, labelsize=16)
#axv[1][2].set_xscale('log')   
axv[1][2].set_xlabel(r'$Log(P) [days] $', fontsize=18)
axv[1][2].set_ylabel(r'$e$', fontsize=18)

#plt.subplot(322)
#axv[2][0].errorbar(sp.mbin, sp.fbinvb, xerr=sp.merr1, yerr=sp.ferr1, label=r'Binaries',fmt='o', color = 'Red')
#axv[2][0].errorbar(mbin,fbinaries, xerr=merr, yerr=0, label='Old',fmt='x')
...
#axv[2][1].errorbar(sp.mbin1, sp.fbinvt, xerr=sp.merr1, yerr=sp.ferr2, label=r'Triplets',fmt='o', color = 'Red')
#axv[2][1].errorbar(mbin,ftriplets, xerr=merr, yerr=0, label='Old',fmt='o')
...
#axv[2][2].errorbar(sp.mbin1, sp.fbinv, xerr=sp.merr1, yerr=sp.ferr, label=r'Multiplets',fmt='o', color = 'Red')
#axv[2][2].errorbar(mbin,fmult, xerr=merr, yerr=0, label='Old',fmt='o')
